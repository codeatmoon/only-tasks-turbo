...
export default function TaskRow({
  ...
  index,
  columns = [],
  statusOptions = ["Not started", "Reviewing", "In-progress", "Complete"],
  ...
  const openLink = () => {
    if (!linkValue) return;
    const href = /^https?:\/\//i.test(linkValue)
      ? linkValue
      : `https://${linkValue}`;
  ...
  const defaultVF: VisibleFields = {
    ...
    status: true,
    link: true,
    dynamics: Object.fromEntries(columns.map((c) => [c.key, true])) as Record<
      string,
      boolean
    >,
  };
  ...
  return (
    <div
      className="group grid items-center gap-2 px-6 py-[3px] bg-white dark:bg-gray-900"
      style={{
        gridTemplateColumns:
          gridTemplate ??
          `28px minmax(280px,1fr) var(--w-assignee,160px) var(--w-due-date,160px) var(--w-priority,120px) var(--w-status,132px) ${columns
            .filter((c) => c.type !== "Link")
            .map((c) => `var(--w-dyn-${sanitize(c.key)}, 100px)`)
            .join(" ")} 44px`,
      }}
    >
      ...
      {vf.assignee && (
        <div
          data-col="assignee"
          className={showAssigneeText ? "w-full" : "w-fit"}
        >
          <TaskDropdown
      ...
      {vf.dueDate && (
        <div
          data-col="due-date"
          className="w-full flex items-center justify-start self-center"
        >
          <span className="text-sm text-gray-500 leading-none dark:text-gray-400">
            {fmtDate(String(local.dueDate ?? ""))}
          </span>
        </div>
      )}
...
function DynamicCell({ column, value }: { column: Column; value: unknown }) {
  const base =
    "w-full text-sm text-gray-700 break-words break-all whitespace-normal dark:text-gray-200";
  switch (column.type) {
    case "Text": {
    ...
    case "Date": {
      ...
      const out =
        d && !isNaN(d.getTime()) ? d.toLocaleDateString() : iso || "-";
      return (
        <span className={base + " text-left block leading-none"}>{out}</span>
      );
    }
    case "Link": {
      ...
      return (
        <button
          type="button"
          className="pill bg-white dark:bg-gray-900 w-full flex items-center justify-center"
          onClick={open}
          title={url || "https://"}
        >
