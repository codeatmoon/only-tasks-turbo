...
export default function ColumnConfigModal({
  ...
  const addCol = () => {
    ...
    setCols((prev) => [
      ...prev,
      { id, name: "New Column", key: `field_${prev.length + 1}`, type: "Text" },
    ]);
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <section>
          ...
          <div className="space-y-3">
              ...
              <div className="flex items-center gap-2" key={c.id}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCols((prev) =>
                      prev.map((pc) =>
                        pc.id === c.id ? { ...pc, key: e.target.value } : pc,
                      ),
                    )
                  }
                />
                <select
                  ...
                  onChange={(e) =>
                    setCols((prev) =>
                      prev.map((pc) =>
                        pc.id === c.id
                          ? { ...pc, type: e.target.value as ColumnType }
                          : pc,
                      ),
                    )
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setCols((prev) => prev.filter((pc) => pc.id !== c.id))
                  }
                >
                  Remove
                </button>
            ...
            <button className="icon-btn" onClick={addCol}>
              Add Column
            </button>
          </div>
        </section>
        ...
        {values.map((v, i) => (
          <div key={`${v}-${i}`} className="flex items-center gap-1">
            <input
              ...
              onChange={(e) =>
                onChange(
                  values.map((ov, oi) => (oi === i ? e.target.value : ov)),
                )
              }
            />
            <button
              className="text-red-600"
              onClick={() => onChange(values.filter((_, oi) => oi !== i))}
            >
              ×
            </button>
          </div>
