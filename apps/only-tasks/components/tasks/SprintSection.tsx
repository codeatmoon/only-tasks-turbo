...
export default function SprintSection({
  ...
  columns?: Column[];
  app?: App;
  onEditTask?: (taskId: string, partial: Partial<Task>) => void;
  ...
  const gridTemplate = useMemo(() => {
    ...
    for (const c of columns) {
      ...
      if (visible.dynamics[c.key] !== false)
        parts.push(`var(--w-dyn-${sanitize(c.key)}, 100px)`);
    }
  ...
  useLayoutEffect(() => {
    ...
    columns.forEach((c) => {
      if (c.type !== "Link") measure(`dyn-${sanitize(c.key)}`);
    });
  ...
  return (
    <section
      ref={sectionRef}
      className={`card overflow-x-auto relative ${showMenu ? "z-[200]" : ""}`}
    >
      <div className="mb-3 flex items-center justify-between relative z-10">
        <span className="inline-block bg-gray-100 px-3 py-1 rounded-full text-sm font-medium dark:bg-gray-800 dark:text-gray-200">
          {sprint.name}
        </span>
        <div className="flex items-center gap-2">
          <button
            className={`icon-btn ${editMode ? "ring-2 ring-indigo-200 dark:ring-indigo-800" : ""}`}
        ...
        {showMenu &&
          (createPortal(
            <>
              {/* Blur overlay */}
              <div
                className="fixed inset-0 z-[210] bg-black/10 backdrop-blur-[2px]"
                onClick={() => setShowMenu(false)}
              />
              {/* Dropdown panel */}
              <div
                className="fixed z-[220] card p-3"
                style={{
                  top: menuPos?.top ?? 0,
                  left: menuPos?.left ?? 0,
                  minWidth: Math.max(menuPos?.width ?? 288, 288),
                  maxHeight: "70vh",
                  overflow: "auto",
                }}
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                  ...
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      className="h-4 w-4"
                      checked={visible.assigneeName}
                      onChange={(e) =>
                        setVisible((prev) => ({
                          ...prev,
                          assigneeName: e.target.checked,
                        }))
                      }
                    />
                        ...
                        .map((c) => (
                          <label key={c.id} className="flex items-center gap-2">
                            <input
                              type="checkbox"
                              className="h-4 w-4"
                              checked={visible.dynamics[c.key] !== false}
                              onChange={(e) =>
                                setVisible((prev) => ({
                                  ...prev,
                                  dynamics: {
      ...
      {/* Header row */}
      <div
        className="text-xs text-gray-500 font-medium mb-2 px-6"
        style={{
          display: "grid",
      ...
      >
        ...
        {columns
          .filter((c) => c.type !== "Link")
          .map((c) =>
            visible.dynamics[c.key] !== false ? (
              <div key={c.id} data-col={`dyn-${sanitize(c.key)}`}>
                {c.name}
              </div>
            ) : null,
          )}
      ...
      <EditTaskModal
        ...
        task={selectedTask}
        onSave={(partial) =>
          selectedTask && onEditTask?.(selectedTask.id, partial)
        }
        options={{
          assignees: app?.options?.assignees,
          status: app?.options?.status,
          priority: app?.options?.priority,
        }}
        columns={columns}
      />
