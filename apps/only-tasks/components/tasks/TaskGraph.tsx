...
export default function TaskGraph({ app }: { app?: App }) {
  ...
  const barKeys: Array<{
    key: keyof (typeof series)[number]["counts"];
    label: string;
    color: string;
  }> = [
  ...
  const donutSegments = (() => {
    const total = Math.max(
      1,
      Object.values(overall.counts).reduce((a, b) => a + b, 0),
    );
    let acc = 0;
  ...
  const topAssignees = useMemo(
    () =>
      Object.entries(overall.assignees)
        .sort((a, b) => b[1] - a[1])
        .slice(0, 6),
    [overall.assignees],
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="card border-l-4 border-gray-300 dark:border-gray-700">
          ...
          <div className="text-2xl font-semibold dark:text-gray-100">
            {overall.total}
          </div>
          <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">
            Across {sprints.length} sprint(s)
          </div>
        </div>
        <div className="card border-l-4 border-emerald-400 dark:border-emerald-700/70">
          ...
          <div className="text-2xl font-semibold dark:text-gray-100">
            {overall.counts.complete}
          </div>
          <div className="text-[11px] text-emerald-600 dark:text-emerald-400 mt-1">
            {Math.round(overall.completionRate * 100)}% completion
          </div>
        </div>
        <div className="card border-l-4 border-pink-400 dark:border-pink-700/70">
          ...
          <div className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">
            Reviewing: {overall.counts.reviewing}
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="card">
          ...
          <div
            ...
            style={{
              gridTemplateColumns: `repeat(${Math.max(series.length, 1)}, minmax(140px, 1fr))`,
            }}
          >
            {series.map((s) => {
              ...
              return (
                <div
                  key={s.id}
                  className="p-2 border rounded-lg dark:border-gray-800"
                >
                  <div
                    className="text-xs font-medium mb-2 truncate dark:text-gray-100"
                    title={s.name}
                  >
                    {s.name}
                  </div>
                  <div className="space-y-1">
                    {barKeys.map((b) => {
                      ...
                      return (
                        <div
                          key={b.key}
                          className="w-full bg-gray-100 rounded h-4 overflow-hidden dark:bg-gray-800"
                        >
                          <motion.div
                            initial={{ width: 0 }}
        ...
        <div className="card">
          <div className="text-sm font-semibold mb-3 dark:text-gray-100">
              ...
              <div className="absolute inset-3 rounded-full bg-white dark:bg-gray-900 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <div className="text-xl font-semibold dark:text-gray-100">
                    {Math.round(overall.completionRate * 100)}%
                  </div>
                </div>
              </div>
            </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card">
          <div className="text-sm font-semibold mb-3 dark:text-gray-100">
            Assignee workload (top {topAssignees.length})
          </div>
          <div className="space-y-2">
            {topAssignees.map(([name, count]) => {
              const pct = overall.total
                ? (Number(count) / overall.total) * 100
                : 0;
              return (
                <div key={name} className="">
                  <div className="flex items-center justify-between text-xs mb-1">
                    <div
                      className="truncate max-w-[60%] dark:text-gray-200"
                      title={name}
                    >
                      {name}
                    </div>
