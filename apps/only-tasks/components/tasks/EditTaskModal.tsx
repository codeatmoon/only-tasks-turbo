...
export default function EditTaskModal({ open, onClose, task, onSave, options, columns = [] }: { open: boolean; onClose: () => void; task?: Task; onSave: (partial: Partial<Task>) => void; options?: { assignees?: string[]; status?: string[]; priority?: string[] }; columns?: Column[] }) {
    ...
    useEffect(() => {
        if (open && task) {
            ...
            columns.forEach(c => { dyn[c.key] = task[c.key] })
            setCustom(dyn)
        }
    }, [open, task, columns])
            ...
            <div className="absolute inset-0 flex items-center justify-center p-4">
                <div className="w-full max-w-lg bg-white rounded-xl shadow-xl border p-5">
                    ...
                    <div className="mt-5 flex justify-end gap-2">
                        ...
                        <button className="px-3 py-2 rounded-lg bg-black text-white" onClick={handleSave} disabled={!name.trim()}>Save</button>
                    </div>
                </div>
            </div>
        </div>
