...
export interface Database {
  ...
  createGlobalUser(user: Omit<DbGlobalUser, 'created_at' | 'updated_at'>): Promise<DbGlobalUser>
  updateGlobalUser(userId: string, updates: Partial<DbGlobalUser>): Promise<DbGlobalUser>
  ...
  createSpace(space: Omit<DbSpace, 'created_at' | 'updated_at'>): Promise<DbSpace>
  ...
  createProject(project: Omit<DbProject, 'created_at' | 'updated_at'>): Promise<DbProject>
  ...
  createSprint(sprint: Omit<DbSprint, 'created_at' | 'updated_at'>): Promise<DbSprint>
  ...
  createCustomField(field: Omit<DbCustomField, 'created_at' | 'updated_at'>): Promise<DbCustomField>

  // App options
  getAppOptions(appId: string): Promise<DbAppOptions[]>
  createAppOptions(options: Omit<DbAppOptions, 'created_at' | 'updated_at'>): Promise<DbAppOptions>

  // Space theme
  getSpaceTheme(spaceId: string): Promise<DbSpaceTheme | null>
  upsertSpaceTheme(theme: Omit<DbSpaceTheme, 'updated_at'>): Promise<DbSpaceTheme>
