  ...
  try {
    ...
    if ('pool' in postgres) {
      ...
      try {
        ...
        `)
        console.log('Existing tables:', existingTables.rows.map((r: TableRow) => r.table_name))
        ...
        if (existingTables.rows.some((r: TableRow) => r.table_name === 'users')) {
          ...
          `)
          console.log('Users table columns:', userColumns.rows.map((r: ColumnRow) => r.column_name))
          
          // Check if the users table has the correct schema
          const hasUserId = userColumns.rows.some((r: ColumnRow) => r.column_name === 'user_id')
          const hasRole = userColumns.rows.some((r: ColumnRow) => r.column_name === 'role')
        ...
        const hasSpaceThemes = existingTables.rows.some((r: TableRow) => r.table_name === 'space_themes')
        if (!hasSpaceThemes) {
