...
export async function initializeSchema(): Promise<void> {
  ...
  try {
    ...
    if (!schema) {
      throw new Error(
        "Could not find schema.sql file in any of the expected locations",
      );
    }
    ...
    console.log(
      `Initializing database schema with ${statements.length} statements`,
    );
    ...
    if ("pool" in postgres) {
      ...
      try {
        ...
        if (existingTables.rows.some((r: any) => r.table_name === "users")) {
          ...
          if (!hasUserId || !hasRole) {
            ...
            for (const table of dependentTables) {
              console.log(
                `Dropping table ${table} to recreate with correct schema`,
              );
        ...
        for (let i = 0; i < statements.length; i++) {
          ...
          try {
            console.log(
              `Executing statement ${i + 1}/${statements.length}: ${statement.substring(0, 100)}...`,
            );
            await client.query(statement);
            console.log(`✓ Executed statement ${i + 1}/${statements.length}`);
          } catch (stmtError) {
            console.error(
              `✗ Error executing statement ${i + 1}: ${statement.substring(0, 200)}...`,
            );
