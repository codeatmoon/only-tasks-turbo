...
export default function DemoPage() {
  ...
  const [lastSel] = useLocalStorage<{ projectId?: string; appId?: string }>('demo-lastSel', {})
  const [view, setView] = useLocalStorage<'sheet' | 'kanban' | 'graph'>('demo-viewMode', 'sheet')
  ...
  const updateCurrentApp = useCallback((updater: (app: AppType) => AppType) => {
  ...
  const onCreateTask = useCallback((sprintId: string, draft: TaskDraft) => {
  ...
  const onEditTask = (sprintId: string, taskId: string, partial: Partial<Task>) => {
    updateCurrentApp(app => ({
      ...app,
      ...
      <div className="max-w-6xl mx-auto">
        <div>
          <header className="flex items-center justify-between mb-6">
            <h1 className="text-2xl font-semibold dark:text-gray-100">Tasks Demo</h1>
