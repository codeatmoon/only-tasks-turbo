...
export default function SettingsPage() {
  ...
  const currentProject = useMemo(
    () => projects.find((p) => p.id === sel.projectId) ?? projects[0],
    [projects, sel],
  );
  const currentApp: AppType | undefined = useMemo(
    () =>
      currentProject?.apps.find((a) => a.id === sel.appId) ??
  ...
  const addApp = (name: string) => {
    ...
    updateProjects((ps) =>
      ps.map((p) =>
        p.id === currentProject.id ? { ...p, apps: [...p.apps, app] } : p,
      ),
    );
    ...
    updateProjects((ps) =>
      ps.map((p) =>
        p.id === currentProject.id
          ? {
              ...p,
              apps: p.apps.map((a) => (a.id === id ? { ...a, name } : a)),
            }
          : p,
      ),
    );
  ...
  const removeApp = (id: string) => {
    ...
    updateProjects((ps) =>
      ps.map((p) =>
        p.id === currentProject.id
          ? { ...p, apps: p.apps.filter((a) => a.id !== id) }
          : p,
      ),
    );
  ...
  const addSprint = (name: string, startDate?: string, endDate?: string) => {
    ...
    if (startDate && endDate) {
      const overlap = (currentApp.sprints || []).some(
        (s) =>
          s.startDate &&
          s.endDate &&
      ...
      ps.map((p) =>
        p.id === currentProject.id
          ? {
              ...
              apps: p.apps.map((a) =>
                a.id === currentApp.id
                  ? { ...a, sprints: a.sprints.filter((s) => s.id !== id) }
                  : a,
              ),
            }
          : p,
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {(projects || []).map((p) => (
            <div key={p.id} className="p-3 border rounded-lg bg-white">
              <InlineEditable
                value={p.name}
                onSave={(n) => renameProject(p.id, n)}
              />
              <div className="flex items-center gap-2 mt-2">
                <button
                  className={`px-2 py-1 rounded text-xs ${sel.projectId === p.id ? "bg-gray-900 text-white" : "bg-gray-100"}`}
                  onClick={() => setSel({ projectId: p.id, appId: undefined })}
                >
                  Select
                </button>
                <button
                  className="ml-auto icon-btn w-8 h-8"
          ...
          <div className="space-y-3">
            {(currentProject?.apps || []).map((a) => (
              <div key={a.id} className="p-3 border rounded-lg">
                <InlineEditable
                  value={a.name}
                  onSave={(n) => renameApp(a.id, n)}
                />
                <div className="flex items-center gap-2 mt-2">
                  <button
                    className={`px-2 py-1 rounded text-xs ${sel.appId === a.id ? "bg-gray-900 text-white" : "bg-gray-100"}`}
                    onClick={() =>
                      setSel({ projectId: currentProject?.id, appId: a.id })
                    }
                  >
                    Select
                  </button>
            ...
            {(currentApp?.sprints || []).map((s) => (
              <div key={s.id} className="p-3 border rounded-lg">
                <div className="flex items-center gap-2">
                  <InlineEditable
                    value={s.name}
                    onSave={(n) => renameSprint(s.id, n)}
                  />
                  {(s.startDate || s.endDate) && (
                    <div className="ml-auto text-xs text-gray-600 flex items-center gap-2">
                      <span className="inline-flex items-center gap-1">
                        <LucideCalendar size={12} /> {s.startDate || "—"}
                      </span>
                      <span>→</span>
                      <span className="inline-flex items-center gap-1">
                        <LucideCalendar size={12} /> {s.endDate || "—"}
                      </span>
                    </div>
                  )}
                </div>
                {!s.startDate && !s.endDate && (
                  <div className="text-[11px] text-gray-500 mt-1">
                    Tip: Add a start/end date to help prevent overlap.
                  </div>
                )}
                <div className="flex items-center gap-2 mt-2">
                  <button
      ...
      {editing ? (
        <>
          <input
            ...
            className="px-2 py-1 border rounded text-sm"
          />
          <button className="icon-btn w-8 h-8" title="Save" onClick={save}>
            <LucideSave size={14} />
          </button>
        </>
      ) : (
